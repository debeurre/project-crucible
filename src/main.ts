...
async function main() {
    ...
    document.body.addEventListener('contextmenu', (e) => e.preventDefault());
        ...
        const resourceSystem = new ResourceSystem(app); // New: ResourceSystem
        ...
        debugContainer.appendChild(scoreElement);
        ...
        worldContainer.addChild(resourceSystem.container); // New: Resource Node visual
        ...
        app.renderer.on('resize', () => {
            ...
            resourceSystem.resize(); // New: Resize resource system
        ...
        let dragStartPos: Point | null = null; // For cut action
        ...
        app.ticker.add((ticker) => {
            ...
            // --- FLOW FIELD PAINTING / CUTTING ---
    
            if (inputState.isDown && !inputState.isHolding) { // Only if mouse is down but NOT holding for spawning
    
                 if (!inputState.isDragging && !dragStartPos) { // Started a new drag for painting/cutting
    
                     dragStartPos = inputState.mousePosition.clone();
    
                 }
    
                
    
                // Calculate drag vector (from previous to current)
    
                const dragVecX = inputState.mousePosition.x - (lastMousePos?.x ?? inputState.mousePosition.x);
    
                const dragVecY = inputState.mousePosition.y - (lastMousePos?.y ?? inputState.mousePosition.y);
    
    
    
                // Only paint if not over crucible
    
                const distSqToCrucible = (inputState.mousePosition.x - crucible.x)**2 + (inputState.mousePosition.y - crucible.y)**2;
    
                const overCrucible = distSqToCrucible < CONFIG.CRUCIBLE_RADIUS**2;
    
    
    
                if (!overCrucible) {
    
                     // Determine if "cut" action (roughly perpendicular to existing flow)
    
                     // For now, let's just make it a dedicated button later. Always paint.
                ...
                dragStartPos = null; // Reset drag start
            ...
            // --- CORE GAME LOOP LOGIC (Resource Management) ---
    
            for (let i = 0; i < sprigSystem.activeSprigCount; i++) {
    
                if (!sprigSystem.isSprigActive(i)) continue; // Should not happen with current setup but good practice
    
    
    
                const sprigBounds = sprigSystem.getSprigBounds(i);
    
    
    
                // 1. Check for PICKUP at Resource Node
    
                if (!sprigSystem.isCarrying(i) && resourceSystem.isInside(sprigBounds.x, sprigBounds.y)) {
    
                    sprigSystem.setCargo(i, 1); // Pick up wood
    
                }
    
    
    
                // 2. Check for DROP-OFF at Crucible
    
                // Use crucible.x/y as center and CRUCIBLE_RADIUS
    
                const dxToCrucible = sprigBounds.x - crucible.x;
    
                const dyToCrucible = sprigBounds.y - crucible.y;
    
                const distSqToCrucible = dxToCrucible*dxToCrucible + dyToCrucible*dyToCrucible;
    
                
    
                if (sprigSystem.isCarrying(i) && distSqToCrucible < (CONFIG.CRUCIBLE_RADIUS + sprigBounds.radius)**2) {
    
                    sprigSystem.setCargo(i, 0); // Drop off cargo
    
                    score++;
    
                    updateDebugUI(); // Update score display
    
    
    
                    // Crucible visual feedback (horizontal squash)
    
                    crucible.scale.set(1.0 + 0.1, 1.0 - 0.1); // Quick squash
    
                    setTimeout(() => crucible.scale.set(1.0, 1.0), 100); // Return to normal
