...
export class SprigSystem {
    ...
    private detectionBuffTimers: Float32Array; // Detection range buff duration
    private detectionBuffValues: Float32Array; // Detection range buff amount
    ...
    constructor(
        ...
        this.detectionBuffTimers = new Float32Array(this.MAX_SPRIG_COUNT);
        this.detectionBuffValues = new Float32Array(this.MAX_SPRIG_COUNT);
    ...
    public spawnSprig(x: number, y: number, team: number = 0) {
        ...
        this.detectionBuffTimers[i] = 0;
        this.detectionBuffValues[i] = 0;
    ...
    public update(ticker: Ticker) {
        ...
        for (let i = 0; i < this.activeSprigCount; i++) { 
            // Update Buff Timer
            if (this.detectionBuffTimers[i] > 0) {
                this.detectionBuffTimers[i] -= dt / 60;
                if (this.detectionBuffTimers[i] <= 0) {
                    this.detectionBuffValues[i] = 0;
                }
            }
    ...
    private updateEusocial(i: number, dt: number) {
        ...
        const detectionRadius = CONFIG.DETECTION_RADIUS + this.detectionBuffValues[i];
        ...
        // Priority 2: Retrieving claimed item
        ...
        // Priority 3: Harvesting active resource
        ...
        // Priority 5: Trace Following (Reverse Flow)
        if (this.cargos[i] === 0) {
    ...
    private removeSprig(i: number) {
        ...
        if (i !== last) {
            ...
            this.detectionBuffTimers[i] = this.detectionBuffTimers[last];
            this.detectionBuffValues[i] = this.detectionBuffValues[last];
